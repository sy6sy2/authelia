...
The key represents the backend attribute name, the `multi_valued` option indicates the value can have multiple values,
and the `value_type` option indicates the type the value must have; either `string`, `integer`, or `boolean`. The
database will be validated given the `multi_valued` and `value_type` configuration.
